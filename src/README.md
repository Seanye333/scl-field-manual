...
plus `FB_HeaterPI` and `FB_ConveyorJam` for graded labs 7 and 8 — both written as
real function blocks rather than the flat lab form, with the parameters (sample time,
manual/auto, travel window) a block on a machine actually needs. Try them yourself
before opening these.
